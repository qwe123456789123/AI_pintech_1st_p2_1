    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(c-> {
            c.requestMatchers("/mypage/**").authenticated()
                    ...
                    .requestMatchers("/admin/**").hasAnyAuthority("MANAGER","ADMIN")
                    // 관리자 페이지를 MANAGER ADMIN 만 접근 가능하게 함
                    .anyRequest().permitAll(); // 나머지  페이지는 모두 접근 가능
        });

        http.exceptionHandling(c ->{
           c.authenticationEntryPoint(new MemberAuthenticationExceptionHandler()) // 미로그인시 인가 실패시
                   .accessDeniedHandler(new MemberAccessDeniedHandler()); // 로그인 이후 인가 실패시
        });
        /* 인가 설정 E */
